...
1.  When space is at a premium, it's useful to label text boxes using a 
    ...
    ```{r, eval = FALSE}
    ui <- fluidPage(
      textInput("your_name", NULL, placeholder = "Your name")
    )
    ```
    :::
...
1.  Carefully read the documentation for `sliderInput()` to figure out how to 
    ...
    We can provide dates to the `min`, `max` and `value` arguments of `fluidPage()`, and tweak the formatting with `timeFormat`.
    ...
    ui <- fluidPage(
      sliderInput(
        ...
        timeFormat = "%F" # the default for dates
      )
    )
    ```
...
1.  If you have a moderately long list, it's useful to create sub-headings that
    ...
    ::: answer
    The solution is to create a named list. Using the example from the `?selectInput()`:
    ```{r}
    ui <- fluidPage(
      selectInput(
        "state",
...
1.  Using the following numeric input box the user can enter any value between 
    ...
    ::: answer
    The `step` argument controls the interval that is suggested when the user clicks on the arrows on the right side of the box. The user is however free to override the suggested values by typing a number directly into the box.
    :::
...
1.  Add an additional plot to the right of the existing plot, and size it so
    ...
    I gave up on this one. I have no idea how to make the plots go side-by-side without resorting to `fluidRow(column())`. I see that `plotOutput()` has an `inline` argument, but it returns `Error in pngfun: invalid quartz() device size`
    ...
    server <- function(input, output, session) {
      output$plot <- renderPlot(plot(1:5))
      output$plot2 <- renderPlot(plot(5:1))
    }
    ```
    
    This fails because the defaults of `width` and `height`, `"100%"` and `"400px"`, are not valid when `inline = TRUE`. These values can be set, but on the server side via `renderPlot()`. Note that in `renderPlot()` the arguments need to be provided as numeric and not character values.
    
    And even after all that, I still can't get the plots to show up side-by-side.
    ...
    server <- function(input, output, session) {
      output$plot <- renderPlot(plot(1:5), height = 300, width = 350)
      output$plot2 <- renderPlot(plot(5:1), height = 300, width = 350)
    }
    ```
    :::
...
1.  Update the options for `renderDataTable()` below so that the table is 
    ...
    app_screenshot(
      testApp(ui, server),
      "basic-ui/sol-output-plot-datatable"
      )
    ```
    ...
    server <- function(input, output, session) {
      output$table <- renderDataTable(
        ...
        options = list(
          pageLength = 5,
          # removes both search box above table and filtering per column
          searching = FALSE,
          # removes arrows next to column names
          ordering = FALSE
          )
        )
    }
    ...
    app_screenshot(
      testApp(ui, server),
      "basic-ui/sol-output-plot-datatable-no-search-order"
      )
    ```
    :::
...
1.  Modify the Central Limit Theorem app so that the sidebar is on the right 
    ...
    app <- testApp(ui, server)
    Sys.sleep(0.5)
    app_screenshot(app, "basic-ui/sol-sidebar-right", width = 800)
    ```
    :::
...
1.  Browse the themes available in the shinythemes package, pick an attractive 
    ...
    app <- testApp(ui, server)
    Sys.sleep(0.5)
    app_screenshot(app, "basic-ui/sol-theme-sandstone", width = 800)
    ```
    :::
