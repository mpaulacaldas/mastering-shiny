...
2.  Make a plot with click, dblclick, hover, and brush output handlers and nicely display the current selection in the sidebar. Plot the plot in the main panel.
    ...
    There is scope for some refactoring in the server code since the arguments to `brushedPoints()` and `nearPoints()` are the same. For example, I could have used `do.call()` to call the appropriate function given the value taken by `input$opts`.
