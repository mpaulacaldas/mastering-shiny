...
::: answer
`r emo::ji("point_up")` **Nice trick:** Pass `validate()` as the last argument to `switch()`.
:::
...
*   In many deployment scenarios, you won't be able to write to the working 
    ...
    ::: answer
    `r emo::ji("question")` I am confused. The code chunk says something 
    different than the words. **TODO**: Try it out with an example.
    :::
...
*   By default, RMarkdown will render the report in the current process, which
    ...
    ::: answer
    `r emo::ji("question")` According to [the article on generating dowloadable reports](https://shiny.rstudio.com/articles/generating-reports.html), we specify `envir = new.envir(parent = globalenv())` to isolate the code in the document from the code in the app. Why isn't this enough? Why do we need the extra call to `callr`? Is it because of the loaded packages and options?
    :::
