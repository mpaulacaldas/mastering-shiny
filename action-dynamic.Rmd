...
::: answer
This strategy is smart (i.e. setting `choices = NULL` to then dynamically update the options). Previously, I would have created the select inputs on the server side and updated the choices there. I think Hadley's approach makes the UI a litte more cluttered (the boxes are there even if you can't use them), but it communicates the range of options better to the user. I also like the fact that it leaves most of the UI code in the ui script.
:::
...
::: answer
The fully fleshed out application can be run from the present repo using:

```{r, eval = FALSE}
shiny::runApp("sales-dashboard/app.R")
```

I particularly like the use of `selectInput(..., selectize = FALSE, size  = 5)`, which changes the appearance of the select input: it creates a larger box that displays 5 options at a time.
:::
...
1.  Complete the user interface below with a server function that updates `input$date` so that you can only select dates in `input$year`.
    ...
    ::: answer
    I also update the default `value` to January 1st of the year defined `input$year`.
    ```{r}
    server <- function(input, output, session) {
      observeEvent(input$year, {
        jan01 <- lubridate::make_date(input$year, 1, 1)
        dec31 <- lubridate::make_date(input$year, 12, 31)
        updateDateInput(session, "date", value = jan01, min = jan01, max = dec31)
      })
    }
    ```
    
    `r emo::ji("bug")` The code above doesn't actually work. This seems to be a bug from Shiny: <https://github.com/rstudio/shiny/issues/2798>
    :::
    ...
    server <- function(input, output, session) {
      observeEvent(input$state, {
        updateSelectInput(
          ...
          choices = county$name[county$state == input$state]
        )
      })
    }
    ```
...
3.  Complete the user interface below with a server function that updates `input$country` choices based on the `input$continent`. Use `output$data` to display all matching rows.
    ...
    server <- function(input, output, session) {
      ...
      output$data <- renderTable({
        subset(continent(), country == input$country)
      })
    }
    ```
    :::
...
4.  Extend the previous app so that you can also choose to select all continents, and hence see all countries. You'll need to add `"(All)"` to the list of choices, and then handle that specially when filtering.
    ...
    <!-- TODO: Update because instructions changed -->
    
    ```{r}
    ui <- fluidPage(
      selectInput("continent", "Continent", choices = c("", levels(continents))), 
      selectInput("country", "Country", choices = NULL),
      tableOutput("data")
    )
    ...
    server <- function(input, output, session) {
      ...
      continent <- reactive({
        if (input$continent == "") return(gapminder)
        subset(gapminder, continent == input$continent)
      })
    ...
    Some remarks:
    - We remove `req(input$continent)` because an empty string (`""`) is considereda "falsy" value by `req()`.
    - `continents` needs to be coerced into a character string before combining it with `""`. Otherwise, we get numeric values in the select input.
...
5.  What is at the heart of the problem described at <https://community.rstudio.com/t/29307>?
    ...
    ::: answer
    Here is the app provided in the question, slightly modified for readability:
    ```{r, eval = FALSE}
    library(shiny)
    ...
    server <- function(input, output,session) {
      observeEvent(input$a, {
        updateNumericInput(session, "b", value = 1 - input$a - input$c) 
        updateNumericInput(session, "c", value = 1 - input$a - input$b) 
      })
      observeEvent(input$b, {
        updateNumericInput(session, "c", value = 1 - input$b - input$a) 
        updateNumericInput(session, "a", value = 1 - input$b - input$c) 
      })
      observeEvent(input$c, {
        updateNumericInput(session, "a", value = 1 - input$c - input$b) 
        updateNumericInput(session, "b", value = 1 - input$c - input$c) 
      })
    }
    ...
    The problem here is that we have inter-related inputs: setting `input$a == .333` (i.e. the value at start-up), triggers a change in `input$a`, `input$b` and `input$c` *at the same time*, which in itself triggers another chain of updates from the same inputs.
    :::
...
::: answer
The app shown before returns the following warning upon start-up:

```r
#> Warning: Error in : Result 1 must be a single string, not NULL of length 0
```

This is because at start-up Shiny tries to create the `output$palette` *before* the `col_names()` inputs have been created (i.e. `input$col1`, `input$col2`, etc.).

I think it is bad practice to leave warnings unaddressed in Shiny apps because they make debugging more difficult. This is how I would adjust the code to get rid of the warning:
...
server <- function(input, output, session) {
  ...
  output$palette <- renderText({
    req(all(col_names() %in% names(input)))
    map_chr(col_names(), ~ input[[.x]])
  })
}
```
...
<!-- TODO: Update because the original code changed -->
Again, here is how I would tweak the code to prevent all warnings. Note that I removed the `%||‰ ""` condition inside `output$col` because I don't know why it's there and I see no problems when I remove it.
```{r}
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    req(all(col_names() %in% names(input))) # inputs must exist
    
    cols <- map_chr(col_names(), ~ input[[.x]])
    cols <- cols[cols %in% colors()] # colors must be valid
    
    req(length(cols) > 0) # vector has appropriate length
...
1.  Take this very simple app based on the initial example in the chapter:
    ...
    ::: answer
    This was my first attempt. I wanted to see if I could create the two inputs in the UI side and give them the same name. It turns out that, in theory, I *can* (the app *looks* fine and I get no warnings), but the value of `input$n` is the one from the input first defined in the page (the slider, in this case).
    ```{r}
    ui <- fluidPage(
    ...
    server <- function(input, output, session) {
      observeEvent(input$type, {
        updateTabsetPanel(session, "switcher", selected = input$type)
      })
      observe(print(input$n))
    }
    ```
    
    The solution is thus to create the inputs in the UI, making sure to give them a unique identifier:
    ```{r}
    ui <- fluidPage(
      tags$style("#switcher { display:none; }"),
      selectInput("type", "type", c("slider", "numeric")),
      tabsetPanel(
        ...
        tabPanel(
          "slider", 
          sliderInput("s", "n", value = 0, min = 0, max = 100)
        ),
        tabPanel(
          "numeric",
          numericInput("n", "n", value = 0, min = 0, max = 100)
    ...
    To keep the values in sync, I use `switch()` and `isolate()` to update the value of the input in the active window with respect to the last value of the input in the inactive window.
    ```{r}
    server <- function(input, output, session) {
      observeEvent(input$type, {
        updateTabsetPanel(session, "switcher", selected = input$type)
        switch(
          input$type,
          "numeric" = updateNumericInput(session, "n", value = isolate(input$s)),
          "slider" = updateSliderInput(session, "s", value = isolate(input$n))
        )
      })
    }
    ```
...
3.  Add support for date and date-time columns `make_ui()` and `filter_var()`.
    ...
    ::: answer
    There is no straight-forward way to deal with the date-time columns, since [Shiny does not currently support date-time input](https://github.com/rstudio/shiny/issues/897). The [shinyTime](https://github.com/burgerga/shinyTime) package adds a time-input, but we can't use it to filter date-time columns without creating an additional date-input. For simplicity, I will treat date and date-time columns the same way in `make_ui()`.
    ...
    make_ui <- function(x, var) {
      if (is.numeric(x)) {
        ...
        dateRangeInput(
          var, var, 
          min = rng[1], max = rng[2], start = rng[1], end = rng[2]
          )
      } else {
        # Not supported
        NULL
    ...
    In `filter_var()`, I only have to make sure that date and date-time columns are filtered in the same way as numeric columns.
    ```{r}
    filter_var <- function(x, val) {
      if (is.numeric(x) || lubridate::is.Date(x) || lubridate::is.POSIXct(x)) {
        !is.na(x) & x >= val[1] & x <= val[2]
      } else if (is.factor(x)) {
        x %in% val
      } else {
    ...
    We can't test this out with the previous app since none of the data frames in the datasets package contain date or date-time columns:
    
    ```{r}
    dfs_ls <- mget(dfs, envir = as.environment("package:datasets"))
    dfs_ls %>%
      map(
        ~ map_lgl(.x, ~ lubridate::is.Date(.x) | lubridate::is.POSIXct(.x))
        ) %>% 
      map_lgl(any) %>% 
      any()
    ```
    
    I will therefore use the `lakers` data set from `lubridate`, doing some processing to get the columns in the right format.
    ...
    ui <- fluidPage(
      sidebarLayout(
        sidebarPanel(
          map(names(lakers), ~ make_ui(lakers[[.x]], .x))
        ),
        mainPanel(
          tableOutput("data")
        )
    ...
    server <- function(input, output, session) {
      selected <- reactive({
        each_var <- map(names(lakers), ~ filter_var(lakers[[.x]], input[[.x]]))
        reduce(each_var, `&`)
      })
      
      # TODO: Format the date and date-time columns for better printing
      output$data <- renderTable(head(lakers[selected(), ], 12))
    }
    ```
