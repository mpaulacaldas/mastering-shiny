...
knitr::include_graphics("demos/require-simple/screenshot.png")
```
...
s1 <- "demos/require-simple2/on-load.png"
s2 <- "demos/require-simple2/langauge.png"
s3 <- "demos/require-simple2/name.png"
...
<div class="answer">
Both `exists()` and `cancelOutput = TRUE` are `r emo::ji("ok_hand")`
</div>
...
<div class="answer">
A pattern that I have seen more often is `validate(need(...))`. Note that with this syntax we have to negate the logical condition defined before.
```{r, eval = FALSE}
server <- function(input, output, server) {
  output$out <- renderText({
    validate(need(
...
<div class="answer">
This is a useful and efficient pattern. Previously, I would have probably used an `observeEvent()` to remove the notification once the import of the file was successful. This approach is better, although I am not 100% clear on why we have to set `add = TRUE`. 

Also, what are the other Shiny functions that return the object's ID? This seems like a useful pattern, but I don't see it documented elsewhere (like `showModal()`, for example).
</div>
...
* Can you make the dangerous option more obvious? Here I've used 
  ...
  <div class="answer">
  `r emo::ji("gem")` More CSS options for buttons at <https://www.w3schools.com/bootstrap/bootstrap_ref_css_buttons.asp>.
  </div>
...
<div class="answer">
Here are some comments explaining what I *think* is going on.
```{r}
runLater <- function(action, seconds = 3) {
  observeEvent(
    # eventExpr -> invalidate after 3 seconds
    invalidateLater(seconds * 1000), 
    # handlerExpr -> called when the event is invalidated
    action, 
    # ignore the handlerExpr (action) the first time it is created
    ignoreInit = TRUE, 
    # destroy after the handlerExpr (action) is run for the first time
    once = TRUE, 
    # the action should be triggered even if the eventExpr is NULL
    ignoreNULL = FALSE,
    # don't auto destroy after the domain ends
    autoDestroy = FALSE
  )
}
...
server <- function(input, output, session) {
  ...
  observeEvent(input$tweet, {
    notification <- glue::glue("Tweeted '{input$message}'")
    # assignment to the global environment, could be replaced with a
    # reactiveValues object?
    last_message <<- input$message
    # remove the message from the text area
    updateTextAreaInput(session, "message", value = "")
    ...
    # weird to use assignement with an observeEvent() instead of an
    # eventReactive()
    waiting <<- runLater({
      cat("Actually sending tweet...\n")
      removeNotification("tweeted")
  ...
  observeEvent(input$undo, {
    # how does this work? can we just call $destroy()  on random objects?
    waiting$destroy()
    showNotification("Tweet retracted", id = "tweeted")
    updateTextAreaInput(session, "message", value = last_message)
  })
