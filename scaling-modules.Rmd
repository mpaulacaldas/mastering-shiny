...
If we do, we will get the following error when we try to run `selectVarApp()`:
...
server <- function(input, output, session) {
  newHistogramServer("hist1", reactive(input$bins))
  output$out <- renderText(paste0("Bins: ", input$bins))
}
# shinyApp(ui, server)
```
...
::: answer
`r emo::ji("gem")` Cool way to mix `radioButtons()` with `selectInput()`.
