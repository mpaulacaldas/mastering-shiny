...
<div class="answer">
`r emo::ji("exploding_head")` Splatting!
</div>
...
<div class="answer">
I think I did something like this once, using `filter_at()` to get around the NSE.
</div>
...
<div class="answer">
Why not use `validate(need(...))` here?
</div>
...
<div class="answer">
`r emo::ji("confused")` I don't get it.
</div>
...
<div class="answer">
`r emo::ji("question")` Do we always have to wrap `callModule()` with a function?
</div>
...
2. Continue working with the same app from the previous exercise, and further 
   ...
   <div class="answer">
   I can't figure out how to do it with a module. Here is an approach that works, with the disadvantage that it is not associated with the UI code.
   
    ```{r}
    server <- function(input, output, session) {
    
      createGgplotVore <- function(myvore, input_x = input$x, input_y = input$y) {
        
        output_name <- paste0("plot_", myvore)
        
        output[[output_name]] <- renderPlot({
          vore_df <- filter(msleep, vore == myvore)
          ggplot(data = vore_df, aes_string(x = input_x, y = input_y)) +
            geom_point()
        })
        
      }
      
      createGgplotVore("carni")
      createGgplotVore("omni")
      createGgplotVore("herbi")
