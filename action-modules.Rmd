...
<div class="answer">
`r emo::ji("exploding_head")` `unname()`

`r emo::ji("question")` Why do we have to wrap `textInput()` with `list()`? Also, confusing that now we use `callModule()` on the server side (although I agree it looks better) and that we have `ns("other")` and `"other"`.

`r emo::ji("bug")` In the app with the module, it is imposible to change to another option after writing something out in the `textInput()`. This was not an issue in the previous app.
</div>
