...
1.  Can the reactive graph contain a cycle? Why/why not?
    ...
    <div class="answer">
    No, it is not possible for a reactive graph to contain a cycle. Since the execution order is determined solely by the reactive graph, Shiny won't know where to start if it ever encounters a reactive component that depends upon itself (either directly or indirectly). We could, however, tell Shiny that a reactive object depends upon *previous* values of the same reactive. One common way of doing this is to use `isolate()` and `reactiveValues()` objects.
    </div>
