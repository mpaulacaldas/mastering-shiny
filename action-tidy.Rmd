...
<div class="answer">
`r emo::ji("gem")` This `.env` solves a problem that I have all the time.
</div>
...
<div class="answer">
This is a clear improvement over `one_of()`, which returned a warning if the columns did not exist in the data frame.
</div>
...
<div class="answer">
This is equivalent to the soon to be superseded `group_by_at(input$vars)`.
</div>
...
<div class="answer">
This is a perfect example of something that was hard to do with the previous scoped dplyr verbs:
```{r, eval = FALSE}
mtcars %>% 
  group_by_at(input$vars_g) %>% 
  transmute_at(input$vars_s, mean) %>% 
  mutate(n = n()) %>% 
  filter(row_number() == 1)
```
</div>
