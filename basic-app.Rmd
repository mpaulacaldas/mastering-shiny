...
::: answer
**TODO** Check why the snippet is not available in my RStudio. I am using  1.2.5033.
:::
...
::: answer
This is a very useful functionality of `ls()`. `ls("package:datasets")` returns all of the exported objects (in this case, all data sets) from the datasets package. Note that for the function to work, the package needs to be part of the search list (i.e. it has to be loaded). There is no need to explicitly load datasets because it is part of base R.
:::
...
::: answer
The first argument to `get()` is a character string with the name of the object, and the second is the environment where to look for the object.
:::
...
1.  Suppose your friend wants to design an app that allows the user to set a 
    ...
    ```{r, echo = FALSE, out.width = NULL}
    app <- testApp(ui, server)
    app_screenshot(app, "basic-app/sol-x-times-5")
    ```
    :::
...
1.  Extend the app from the previous exercise to allow the user to set the 
    ...
    ```{r, echo = FALSE, out.width = NULL}
    app <- testApp(ui, server)
    app_screenshot(app, "basic-app/sol-x-times-y")
    ```
...
1.  Replace the UI and server components of your app from the previous exercise 
    ...
    ::: answer
    The app takes two inputs, `x` and `y`, and returns the results of three different operations: `x * y`, `(x * y) + 5` and `(x * y) + 10`. Note that the second and third operations can be calculated from the first. We can get rid of this duplication with a reactive.
    ```{r}
    ui <- fluidPage(
      sliderInput("x", "If x is", min = 1, max = 50, value = 30),
      sliderInput("y", "and y is", min = 1, max = 50, value = 5),
    ...
    ```{r, echo = FALSE, out.width = NULL}
    app <- testApp(ui, server)
    app_screenshot(app, "basic-app/sol-x-times-y-plus")
    ```
    :::
...
1.  The following app is very similar to one you've seen earlier in the chapter: 
    ...
    - Regular typo: `output$summmry` should be `output$summary`
    - Forgot to call the reactive: `plot(dataset)` should be `plot(dataset())`
    - Wrong output type: `tableOutput("plot")` should be `plotOutput("plot")`
    
    <br/>
    Note that here I launched the app only with `ggplot2::seals`. This is because the calls to `plot()` take too long to render with other data sets.
